...
export class UserService {
    ...
    private async getAccessToken(): Promise<string> {
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response) {
                ...
                throw new UnauthorizedException(`Azure AD token request failed: ${data.error_description || data.error}`);
            } else {
                this.logger.error('Unexpected error while fetching Azure AD token', error);
                throw new UnauthorizedException('Could not retrieve access token from Azure AD');
            }
        }
    }
    ...
    private async createAzureUser(data: any): Promise<any> {
        ...
        try {
            ...
            this.logger.log(`Azure AD user created successfully: ${data.userPrincipalName}`);
            return response.data;
        } catch (error) {
            console.log("error", error)

            this.logger.error('Failed to create Azure AD user', error.response?.data || error);
    ...
    private async deleteAzureUser(azureId: string): Promise<void> {
        ...
        try {
            ...
            await axios.delete(
                `https://graph.microsoft.com/v1.0/users/${azureId}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            this.logger.error(`Failed to delete Azure AD user with ID: ${azureId}`, error.response?.data || error);
    ...
    private async updateAzureUser(azureUserId: string, data: any): Promise<void> {
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response) {
                ...
                throw new Error(data.error?.message || 'Failed to update user in Azure AD');
            } else {
                this.logger.error('Unexpected error while updating Azure AD user', error);
                throw new Error('Unexpected error occurred while updating Azure AD user.');
            }
        }
    }
    ...
    async updateUser(userId: string, data: any) {
        ...
        try {
            ...
            // Update Azure AD user if email is provided
            if (data?.email) {
                await this.updateAzureUser(existingUser.azureId, data);
            }
            ...
            // Update user details if any fields other than permissions are provided
            if (Object.keys(updateData).length > 0 && !data.permissions) {
                transactions.push(
            ...
            // Update user-specific permissions if provided
    ...
    async azureRedirect(code: string, res: Response) {
        try {
            ...
            if (!res.headersSent) {
                ...
                res.status(500).json({ message: 'Azure sign-in failed' });
            } else {
